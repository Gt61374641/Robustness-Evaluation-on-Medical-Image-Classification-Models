"""Generate per-model configs for the ResNet complexity ladder.

Reads each `configs/{dataset}_base.yaml` and writes `configs/{dataset}_{model}.yaml`
for every ResNet in the ladder, changing ONLY the model name. This guarantees the
eps grid, training block and class-balance settings are byte-identical across the
ladder — a prerequisite for a fair complexity comparison.

Usage:
    python scripts/make_configs.py            # regenerate all 15 configs
"""
